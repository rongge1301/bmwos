			...
			<transition name="fade">
				<div class="select_box" v-if="select_show">
					...
					<div class="set_main">
						...
						<div class="set_main_box">
							<p>Publisher(EN))</p>
							<el-select v-model="selecteds.publisherEn" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select5">
							    <el-option
				...
				<div id="test_box" v-if="list_show">
					<div v-if="detail_show" class="detail_box">
						<div class="detail_box_box">
							<a @click="showDetail()">
							<!-- 图片 -->
							<img src="@/assets/img/Newspaper.png" alt="" height="100%"> 
							</a>
							<p>
					 ...
					 <div v-if="detail_close" class="detail_box">
					 	<el-tabs type="card">
							  <el-tab-pane>
								  ...
								  <detail-List :selected="'Checking'" :category="'Newspaper'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							  <el-tab-pane>
			                      <span slot="label">Rejected List</span>
			                      <detail-List :selected="'Rejected'" :category="'Newspaper'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							   <el-tab-pane>
			                      <span slot="label">Approved List</span>
			                      <detail-List :selected="'Approved'" :category="'Newspaper'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
				...
				<div id="table_box" v-if="show">
					...
					<div class="table_box_item1">
						...
						<div class="table_box_item1_item" style="margin-left:30px;">
							...
							<p>Size/Format: <span>{{item_box.media_list}}</span></p>
...
export default {
  ...
  methods:{
  	onloadList(){
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/nsp/get_option.do',
            data:this.selecteds
  		}).then(response => {
  	...
  	Confirm(){
  		// JJ
  		this.tags = [{
  			name: this.selecteds.grade +' > '+ this.selecteds.year +' > '+ this.selecteds.province +' > '+ this.selecteds.cityRegion +' > '+ this.selecteds.publisherEn +' > '+ this.selecteds.section +' > '+ this.selecteds.dealDate, type: ''
  		}];
  		this.show =false;
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/nsp/get_detail.do',
            data:this.selecteds
  		}).then(response => {
