				...
				<div class="select_box" v-if="select_show">
					...
					<div class="set_main">
						...
						<div class="set_main_box">
							...
							<p>Province</p>
							<el-select v-model="selecteds.province" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select3">
							    <el-option
							      v-for="item in Options_List.province"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
							...
							<p>City / Region</p>
							<el-select v-model="selecteds.cityRegion" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select4">
							    <el-option
							      v-for="item in Options_List.cityRegion"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
							...
							<p>Channel</p>
							<el-select v-model="selecteds.channelCh" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select5">
							    <el-option
							      v-for="item in Options_List.channelCh"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
						...
						<div class="set_main_box">
							<p>Program</p>
							<el-select v-model="selecteds.program" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select6">
							    <el-option
							      v-for="item in Options_List.program"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
				...
				<div id="test_box" v-if="list_show">
					<div v-if="detail_show" class="detail_box">
						<div class="detail_box_box">
							<a @click="showDetail()">
							<!-- 图片 JJ-->
							<img src="@/assets/img/TV.png" alt="" height="100%"> 
							</a>
							<p>
					 ...
					 <div v-if="detail_close" class="detail_box">
					 	<el-tabs type="card">
							  <el-tab-pane>
								  ...
								  <detail-List :selected="'Checking'" :category="'TV'" @check="checkDetail" ></detail-List>
							  </el-tab-pane>
							  <el-tab-pane>
			                      <span slot="label">Rejected List</span>
			                      <detail-List :selected="'Rejected'" :category="'TV'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							   <el-tab-pane>
			                      <span slot="label">Approved List</span>
			                      <detail-List :selected="'Approved'" :category="'TV'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
				...
				<div id="table_box" v-if="show">
					...
					<div class="table_box_item1">
						...
						<div class="table_box_item1_item" style="margin-left:30px;">
							...
							<p>30" Net Cost (incl. loading fee) (RMB): <span>{{item_box.commitment |formatNull}}</span></p>
							<p>Position Loading Fee: <span>{{item_box.position_loading_fee | formatPercent}}</span> </p>
...
export default {
  ...
  methods:{
  	onloadList(){
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/tv/get_option.do',
            data:this.selecteds
  		}).then(response => {
  	...
  	Confirm(){
  		// JJ
  		this.tags = [{
  			name: this.selecteds.grade +' > '+ this.selecteds.year +' > '+ this.selecteds.province +' > '+ this.selecteds.cityRegion +' > '+ this.selecteds.channelCh +' > '+ this.selecteds.program +' > '+ this.selecteds.section +' > '+ this.selecteds.dealDate, type: ''
  		}];
  		this.show =false;
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/tv/get_detail.do',
            data:this.selecteds
  		}).then(response => {
