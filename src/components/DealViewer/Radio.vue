					 ...
					 <div v-if="detail_close" class="detail_box">
					 	<el-tabs type="card">
							  <el-tab-pane>
								  ...
								  <detail-List :selected="'Checking'" :category="'Radio'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							  <el-tab-pane>
			                      <span slot="label">Rejected List</span>
			                      <detail-List :selected="'Rejected'" :category="'Radio'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							   <el-tab-pane>
			                      <span slot="label">Approved List</span>
			                      <detail-List :selected="'Approved'" :category="'Radio'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
...
export default {
  ...
  methods:{
  	onloadList(){
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/rdo/get_option.do',
            data:this.selecteds
  		}).then(response => {
  	...
  	Confirm(){
  		// JJ
  		this.tags = [{
  			name: this.selecteds.grade +' > '+ this.selecteds.year +' > '+ this.selecteds.province +' > '+ this.selecteds.cityRegion +' > '+ this.selecteds.publisherType +' > '+ this.selecteds.publisherEn +' > '+ this.selecteds.section +' > '+ this.selecteds.dealDate, type: ''
  		}];
  		this.show =false;
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/rdo/get_detail.do',
            data:this.selecteds
  		}).then(response => {
