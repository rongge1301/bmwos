			...
			<transition name="fade">
				<div class="select_box" v-if="select_show">
					...
					<div class="set_main">
						...
						<div class="set_main_box">
							<p>Supplier(CH)</p>
							<el-select v-model="selecteds.supplierCh" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select6">
							    <el-option
							      v-for="item in Options_List.supplierCh"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
						...
						<div class="set_main_box">
							<p>Format(CH)</p>
							<el-select v-model="selecteds.formatCh" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select6">
							    <el-option
							      v-for="item in Options_List.formatCh"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
						...
						<div class="set_main_box">
							<p>Address</p>
							<el-select v-model="selecteds.address" filterable clearable v-on:change="changeList()" placeholder="Please select" id="select6">
							    <el-option
							      v-for="item in Options_List.address"
							      :key="item.label"
							      :label="item.label"
							      :value="item.label">
							    </el-option>
				...
				<div id="test_box" v-if="list_show">
					<div v-if="detail_show" class="detail_box">
						<div class="detail_box_box">
							<a @click="showDetail()">
							<!-- 图片 -->
							<img src="@/assets/img/OOH.png" alt="" height="100%"> 
							</a>
							<p>
					 ...
					 <div v-if="detail_close" class="detail_box">
					 	<el-tabs type="card">
							  <el-tab-pane>
								  ...
								  <detail-List :selected="'Checking'" :category="'OOH'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							  <el-tab-pane>
			                      <span slot="label">Rejected List</span>
			                      <detail-List :selected="'Rejected'" :category="'OOH'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
							   <el-tab-pane>
			                      <span slot="label">Approved List</span>
			                      <detail-List :selected="'Approved'" :category="'OOH'" @check="checkDetail"></detail-List>
							  </el-tab-pane>
				...
				<div id="table_box" v-if="show">
					...
					<div class="table_box_item1">
						...
						<div class="table_box_item1_item" style="margin-left:30px;">
							...
							<p>Net Cost (excl. production cost) (RMB): <span>{{item_box.commitment |formatNull}}</span></p>
...
export default {
  ...
  methods:{
  	onloadList(){
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/ooh/get_option.do',
            data:this.selecteds
  		}).then(response => {
  	...
  	Confirm(){
  		// JJ
  		this.tags = [{
  			name: this.selecteds.grade +' > '+ this.selecteds.year +' > '+ this.selecteds.province +' > '+ this.selecteds.cityRegion +' > '+ this.selecteds.publisherType +' > '+ this.selecteds.supplierCh +' > '+ this.selecteds.formatCh +' > '+ this.selecteds.address+' > '+ this.selecteds.dealDate, type: ''
  		}];
  		this.show =false;
  		this.$http({
  			method:'post',
            url:'http://114.55.92.124:9090/bmwos/deal/ooh/get_detail.do',
            data:this.selecteds
  		}).then(response => {
